      ...
      <div className="relative min-h-screen">
        ...
        <div className="container mx-auto px-4 pt-24 pb-10 relative z-10">
          <div className="mx-auto max-w-6xl space-y-10">
            <div className="rounded-3xl bg-white/85 p-8 drop-shadow-lg ring-1 ring-gray-100">
              ...
              {(currentUser.badges && currentUser.badges.length > 0) || currentUser.badge ? (
                <div className="mt-6 flex flex-wrap gap-2">
                  {currentUser.badges && currentUser.badges.length > 0 ? (
                    ...
                    currentUser.badge && (
                      <span className="inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold" style={{ backgroundColor: `${themeColor}15`, color: themeColor }}>
            ...
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-3xl bg-white/85 p-8 drop-shadow-lg">
                ...
                {userPosts.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-gray-200 p-6 text-center text-gray-500">
                    まだ投稿がありません。<br />
                    <a href="/posts/new" className="underline" style={{ color: themeColor }}>
                      冒険日誌を投稿
                    </a>
                    してみましょう。
                  </div>
                  ...
                  <div className="space-y-6">
                    {userPosts.map((post, index) => (
                      <PostCard key={post.id} post={post} chapterNumber={userPosts.length - index} themeColor={themeColor} />
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-6">
                <div className="rounded-3xl bg-white/85 p-8 drop-shadow-lg">
                  ...
                  {nextStepPosts.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-gray-200 p-4 text-sm text-gray-500">
                      ...
                      ボランティア募集ページから
                      <span className="font-semibold" style={{ color: themeColor }}>「投稿を保存」</span>
                      ボタンを押すと、ここに表示されます。
                    </div>
                  ) : (
                ...
                <div id="friends" className="rounded-3xl bg-white/85 p-8 drop-shadow-lg">
                  <h2 className="text-2xl font-bold text-gray-900 mb-4">旅の仲間リスト</h2>
                  {friends.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-gray-200 p-4 text-sm text-gray-500">
                      まだ旅の仲間が登録されていません。<br />
                      気になるユーザーのページから
                      <span className="font-semibold" style={{ color: themeColor }}>「旅の仲間リストに加える」</span>
                      ボタンを押してみましょう。
                    </div>
                  ) : (
                ...
                <div className="rounded-3xl bg-white/85 p-8 drop-shadow-lg space-y-4">
                  ...
                  <a
                    href="/mypage/profile"
                    className="inline-flex items-center justify-center rounded-full px-6 py-2 text-sm font-semibold text-white shadow hover:opacity-90 transition"
